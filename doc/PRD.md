...
- ❌ **Web Share API** (testée v2.3, retirée v2.3.x) : la feuille de partage natif Safari/Chrome perturbait le flow de téléchargement attendu (PDF dans le dossier Téléchargements + ouverture optionnelle du mailto). Retour à `doc.save()` direct partout. Module et tests supprimés.
